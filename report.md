...
実装したアルゴリズムにおいて2点間の最短経路が探索される流れを以下に示す．
...
先頭の要素をキューから取り出し，その要素に接続された全ての子ノードについて既に探索済みであるかどうかを調べる．そのうち探索済みでない子ノードについて，探索済みの印を付け，親ノードと共にキューの末尾に追加していく．この動作をゴールノードが発見されるまで繰り返す．ゴールノードが発見されると，その時点におけるキューの値を返す．このときキューの最後尾の要素には，スタートノードからゴールノードまでの最短経路が格納されているので，最終的にその値を ```Creating path:``` として出力する．  
